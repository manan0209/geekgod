...
function partition(array, startIdx, endIdx):
    ...
    return i + 1`}
        ...
        <div className="flex justify-center gap-4 mb-6">
          <button
            onClick={() =>
              updateDataset([10, 15, 20, 5, 30, 25, 50, 45, 35, 40])
            }
          ...
          <button
            onClick={() =>
              updateDataset([19, 33, 17, 21, 15, 6, 8, 20, 14, 10])
            }
            className="px-4 py-2 bg-blue-500 text-white rounded"
          >
            Dataset 2
...
<div className="w-full lg:w-1/2 bg-gray-800 rounded-lg shadow-lg p-6 relative">
  <h2 className="text-2xl font-bold text-center mb-4 text-gray-200">C++ Code</h2>
...
<div className="w-full lg:w-1/2 bg-gray-800 rounded-lg shadow-2xl p-8">
  ...
  <p className="text-lg leading-relaxed text-gray-300 mb-6">
    Quick Sort is a highly efficient, divide-and-conquer sorting algorithm. It works by selecting a pivot element from the array, partitioning the array into two subarrays based on the pivot, and recursively sorting the subarrays. On average, it achieves a time complexity of O(n log n), making it one of the fastest algorithms for large datasets. Its ability to sort in place (without needing additional storage) is another key benefit.
  </p>
  ...
  <div className="text-gray-300">
    ...
    <ul className="list-inside list-disc mb-6">
      <li>
        <strong>1. Choose a Pivot:</strong> The first step is to choose a pivot element from the array. Common strategies include picking the first element, the last element, or a random element.
      </li>
      <li>
        <strong>2. Partition the Array:</strong> Rearrange the array by moving elements less than the pivot to the left and elements greater than the pivot to the right. This creates two subarrays where all elements to the left of the pivot are smaller, and all elements to the right are larger.
      </li>
      <li>
        <strong>3. Recursively Sort the Subarrays:</strong> Apply the same process to the left and right subarrays, choosing new pivots for each one. The process continues until the subarrays are small enough to be considered sorted.
      </li>
      <li>
        <strong>4. Repeat Until Sorted:</strong> The algorithm repeats these steps for each subarray until no further partitioning is needed, resulting in a fully sorted array.
      </li>
    </ul>
    ...
    <ul className="list-inside list-disc mb-6">
      <li>
        <strong>Best Case:</strong> <span className="text-blue-400">O(n log n)</span> — Occurs when the pivot divides the array into nearly equal halves each time, ensuring optimal sorting performance.
      </li>
      <li>
        <strong>Average Case:</strong> <span className="text-orange-400">O(n log n)</span> — This is the typical performance for random data and is highly efficient for large datasets.
      </li>
      <li>
        <strong>Worst Case:</strong> <span className="text-red-400">O(n^2)</span> — The worst case happens when the pivot choice leads to unbalanced partitions, such as when the array is already sorted or nearly sorted. This can be avoided by using techniques like randomized pivot selection.
      </li>
    </ul>
    ...
    <ul className="list-inside list-disc mb-6">
      <li><strong>In-Place Sorting:</strong> Quick Sort does not require extra space for another array, unlike Merge Sort, making it more space-efficient.</li>
      <li><strong>Efficiency for Large Datasets:</strong> Despite its worst-case time complexity, Quick Sort is often faster than other algorithms like Merge Sort due to its smaller constant factors and cache efficiency.</li>
      <li><strong>Parallelizable:</strong> Its recursive nature makes it well-suited for parallel processing on modern multi-core processors.</li>
    </ul>
  </div>

  <p className="text-gray-400 italic mt-6 text-center">
    "Brought to you by <span className="text-blue-500">GeekGod</span> – turning algorithms into child's play!"
  </p>
</div>
